...
class TestExportResumeMisc(BoardCase):
    ...
    def test_resume_and_orphans(self):
        ...
        self.to_doing(live, agent='live1234567')
        self.to_doing(dead, agent='dead7654321')
        ...
        with mock.patch.object(board, 'agent_running', side_effect=lambda a: a.startswith('live')) as m:
            out = self.ok('resume').splitlines()
        self.assertEqual(sorted(c.args[0] for c in m.call_args_list), ['dead7654321', 'live1234567'])
        self.assertEqual(len(out), 4)
        self.assertRegex(out[0], rf'^{live} doing eng5 live123 \d+m T-L handoff=- \| Live \| .* lead status ready->doing$')
        ...
        self.assertEqual(out[3], f'orphan {dead} dead765 T-D no running transcript')
