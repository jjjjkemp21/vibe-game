	...
	const FVector2D& Direction, float Distance, const ULureFishingSettings& Settings)
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(LureCastLanding), false, IgnoreActor);
	...
	if (World)
	{
		...
		FHitResult Hit;
		if (World->LineTraceSingleByObjectType(Hit, Origin, FVector(TargetXY.X, TargetXY.Y, AimZ), Objects, Params) && !Hit.bStartPenetrating)
		{
	...
	FHitResult Ground;
	const bool bGround = World && World->LineTraceSingleByObjectType(Ground, FVector(TargetXY.X, TargetXY.Y, TopZ), FVector(TargetXY.X, TargetXY.Y, BottomZ), Objects, Params);
