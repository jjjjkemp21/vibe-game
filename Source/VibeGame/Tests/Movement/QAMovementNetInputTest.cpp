...
bool FQAMoveInputAllSixActionsResolveByName::RunTest(const FString& Parameters)
{
	...
	// QA (T-006 review): the list is EXACTLY the known actions - the 6 movement actions, T-010's Interact, T-006's Cast and Hook
	// (tested in Project.Fishing.QA.Input.*) and T-028's ReelFaster / ReelSlower (Project.Fishing.Fight.Rod.InputRoutesLookToTheRod).
	// A new action must be added here on purpose; an unexpected, duplicate or renamed action fails.
	const TArray<FName> Known = { TEXT("Move"), TEXT("Look"), TEXT("Jump"), TEXT("Sprint"), TEXT("Crouch"), TEXT("Prone"), TEXT("Interact"), TEXT("Cast"), TEXT("Hook"),
		TEXT("ReelFaster"), TEXT("ReelSlower") };
	TestEqual(TEXT("GetInputActionNames lists exactly the known actions (6 movement + Interact + Cast + Hook + ReelFaster + ReelSlower)"), Listed.Num(), Known.Num());
	TSet<FName> Unique;
