...
- DataTables (T-072, 2026-09-24): `tools/integrate.ps1` re-imports every changed `data/tables/DT_*` source headless in the batch lane, after the build and before the tests, then commits the .uasset there. It refuses a fast-forward that changes assets while the main editor is open, so close the editor before integrating data changes. Brand-new tables are imported too (`import_new_table`, row struct found by the F<Name>Row / FLure<Name>Row convention; first real use in S5, check its log line). In a lane, run-tests.ps1 re-imports changed tables for the run only. `-ExpectFail <exact test paths>` lets an integration accept named failures that can only pass after an editor step in main (e.g. new ABP pins for a new enum value); then do the editor step and rerun the full suite in main before any push. In main with the editor closed, call tools/unreal-python.ps1 from the PowerShell tool (pwsh 7), not from bash: bash->PS 5.1 mangles the JSON quotes.
...
## Working with Jimmy
- Editor frame cap (Jimmy, 2026-09-24): while agents use the editor it runs at 30 fps (`tools/launch-editor.ps1` passes `t.MaxFPS 30`; the default -MaxFps is 30). Before handing a build to Jimmy, set it back to normal in the running editor through unreal-mcp run_python: `unreal.SystemLibrary.execute_console_command(None, 't.MaxFPS 0')` (0 = uncapped, the engine default). Set it back to 30 when he's done playing. Playtesters who measure performance lift the cap only for that measurement.
...
- Sprints (Jimmy, 2026-09-24): split his playtest notes into themed sprints (board objectives S1..Sn), core fishing feel first. Departments work ahead in parallel; each sprint ends with a build, the release gate and a new feedback file. Multiplayer is tested but never blocks a sprint build or the gate: 2P issues go to board objective MP, and the code stays server-authoritative.
