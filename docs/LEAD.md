...
- DataTables (T-072, 2026-09-24): `tools/integrate.ps1` re-imports every changed `data/tables/DT_*` source headless in the batch lane, after the build and before the tests, then commits the .uasset there. It refuses a fast-forward that changes assets while the main editor is open, so close the editor before integrating data changes. Brand-new tables still need the editor-operator (`import_datatable` with the row struct).
