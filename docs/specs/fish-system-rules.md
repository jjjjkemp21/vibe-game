...
## Confirmed implementation choices (lead, after QA report 20260922-2346-eng2-T008)
- Replication size (T008-B1): GameplayTags `FastReplication=True` in `Config/DefaultGameplayTags.ini` (tags go over the wire as an index, not a name). Client and server must have the same tag list; they do, since every tag is in `Config/Tags/*.ini`. FFishInstance itself is unchanged.
- Runtime data warnings (O2): a bad row met during a roll (skipped mod, invalid RollWeight/RollChance, unknown allowed rarity, invalid weight range, no Weight stat row, a species that can roll no rarity, ...) is logged once per (table object, row, problem) per session; the roll behaves the same every time. The seen-set is bounded (1024 keys, then emptied). Caller problems (missing tables, unknown species, NaN luck or forced weight, unknown forced ids) warn on every call. `FFishRoll::ResetDataWarnings()` clears it (tests, debug tools).
- Forced overrides (O3): an unknown ForcedRarityId, or any unknown id in ForcedModifierIds, fails the whole roll with a Warning naming the id (no partial fish). Forced overrides are for tests and debug tools (T-025 `Lure.GiveFish`) only.
- Validator (O4): among enabled tiers (RollWeight > 0), XpMultiplier must not decrease with Rank (ties allowed), reported like the ValueMultiplier check.
