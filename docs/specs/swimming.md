...
- **Tuning** is in DT_Movement: rows `Swim` and `SwimSprint` (speed, acceleration, eye height from the feet, camera blend
  time, noise multiplier for T-014) plus three water columns (0 on land rows, optional in the CSV):
  `SurfaceFloatDepth` (capsule center below the surface, cm), `ClimbOutMaxHeight` (cm above the water),
  `ClimbOutSpeed` (cm/s). Shipped: swim 170 / sprint-swim 290 cm/s, eyes 18 cm above the water, edges up to 60 cm.
...
- **Getting out:** walk out where the seabed rises (beaches, ramps, shelves). Or press **Jump** facing an edge within
  45 cm: if its top is at most `ClimbOutMaxHeight` above the water (60 cm; 61 cm is refused) and there is room to stand,
